...
void CPlayerPed::FireInstant()
{
    ...
    if (GetCurrentWeapon() == WEAPON_SNIPERRIFLE)
    {
        // CWeapon::FireSniper(CWeapon* thiz, CPed* owner, int unk)
        ((void (*)(CWeapon*, CPedGTA*, int))(g_libGTASA + (VER_x32 ? 0x005663E0 + 1 : 0x692A28)))(pSlot, m_pPed, 0);
    }
    else
    {
        // CWeapon::FireInstantHit(CWeapon* thiz, CPed* owner, int unk, CVector* origin, int unk2)
        ((void (*)(CWeapon*, CPedGTA*, int, CVector*, int))(g_libGTASA + (VER_x32 ? 0x00566190 + 1 : 0x69269C)))(pSlot, m_pPed, 0, nullptr, 0);
    }

    if(m_bytePlayerNumber == 0 && m_pPed) {
        m_pPed->bFiringWeapon = true;
    }

    // Decrementar munição e processar som (chamando a lógica original do jogo via FireInstantHit/FireSniper)
    if (m_bytePlayerNumber == 0 && pSlot && pSlot->dwType != WEAPON_UNARMED)
    {
        if (pSlot->dwAmmoInClip > 0) 
            pSlot->dwAmmoInClip--;
        
        if (pSlot->dwAmmo > 0)
            pSlot->dwAmmo--;
            
        // Se a munição acabar, o jogo nativo cuidará do estado WEAPONSTATE_OUT_OF_AMMO
        // ao processar o próximo frame ou via lógica interna do CWeapon::Fire
    }
