...
//#define AUTH_BS "3952314F524834355456464955525858554F3353"

extern UI* pUI;
extern CGame* pGame;
extern CAudioStream* pAudioStream;
//extern Channel* pVoice;
...
#define RPC_CUSTOM_SET_HANDLING		0x32
...
void CNetGame::Packet_CustomRPC(Packet *p) {
    ...
    case RPC_CUSTOM_SET_HANDLING:
    {
        Log("RPC_CUSTOM_SET_HANDLING");
        ...
        if (pVehiclePool)
        {
            if (pVehiclePool->GetAt(veh))
            {
                pVehiclePool->GetAt(veh)->SetHandlingData(comps);
            }
        }
    }
...
void CNetGame::ShutdownForGameModeRestart()
{
	...
	if (pUI) pUI->chat()->addInfoMessage("Сервер перезагружается...");
}
...
{
	if(pUI) pUI->chat()->addDebugMessage("Сервер переполнен. Повторная попытка...");
...
{
	if (pUI) pUI->chat()->addDebugMessage("Сервер закрыл соединение.");
	if (pAudioStream) {
		pAudioStream->Stop(true);
	}
...
void CNetGame::Packet_ConnectionSucceeded(Packet *pkt)
{
	...
	if (pUI) pUI->chat()->addDebugMessage("Подключено. Присоединяюсь к игре...");
...
{
	if (pUI) pUI->chat()->addDebugMessage("Вам запрещено находиться на этом сервере.");
}
// 0.3.7
...
void CNetGame::cp1251_to_utf8(char *btn, char *buff) {
